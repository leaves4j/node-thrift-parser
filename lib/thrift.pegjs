...
Const           
  =  'const' _ filedType:FieldType _ identifier:Identifier _ '=' _ value:ConstValue _? ListSeparator? {
    return {
      type: 'const',
      filedType,
      identifier,
...
//Field
Field          
  =  __ id: FieldID? option: FieldReq? fieldType: FieldType _ identifier: Identifier defaultValue: (_ '=' _ v: ConstValue {
    return v
...
Function      
  =  __ oneway:(v:'oneway' _ {
    return v
  })? functionType:FunctionType _ identifier:Identifier _? '(' args: Field* ')' throws:Throws? ListSeparator? __ {
    return {
      type: 'function',
      functionType,
      identifier,
...
ConstValueWithoutDouble
  = Literal / Identifier / ConstMap / ConstList / IntConstant

IntConstant = __ v:Int __ {return v}

DoubleConstant 
  = __ sign: ('+' / '-')? digits: Digit* decimal: ('.' decimalDigits: Digit+ __ {
...
ConstMap        
  = __ '{' values: (key: ConstValue ':' value: ConstValue ListSeparator? __{
    return {key, value}
  })* '}' {
    return values
  } 
...
ConstList       
  = __ '[' values: (v: ConstValueWithoutDouble ListSeparator? {return v})* ']' __ {
    return values
  }
